...
- 那么父类的方法声明和方法主体，只有声明还有意义，而方法主体则没有存在的意义了(因为子类对象会调用自己重写的方法)。
- 换句话说，父类可能知道子类应该有哪个功能，但是功能具体怎么实现父类是不清楚的（由子类自己决定），父类完全只需要提供一个没有方法体的方法签名即可，具体实现交给子类自己去实现。**我们把没有方法体的方法称为抽象方法。Java语法规定，包含抽象方法的类就是抽象类**。
...
- **抽象类体现的是模板思想**，模板是通用的东西抽象类中已经是具体的实现（抽象类中可以有成员变量和实现方法），而模板中不能决定的东西定义成抽象方法，让使用模板（继承抽象类的类）的类去重写抽象方法实现需求，这是典型的模板思想。
...
## 2.1 概述
我们已经学完了抽象类，抽象类中可以用抽象方法，也可以有普通方法，已经构造器，成员变量等。那么什么是接口呢？
...
​       注意：接口中的**抽象方法**默认会自动加上`public abstract`修饰程序员无需自己手写！！
​       按照规范：以后接口中的抽象方法建议不要写上`public abstract`。因为没有必要啊，默认会加上。

### 2.3.2 常量

- 在接口中定义的成员变量默认会加上： `public static final`修饰。
- 也就是说在接口中定义的成员变量实际上是一个常量。这里是使用`public static final`修饰后，变量值就不可被修改，并且是静态化的变量可以直接用接口名访问，所以也叫常量。
...
- 接口中的方法全是抽象方法，默认会自动加上`public abstract`修饰
...
我们已经学完了成员变量，构造器，成员方法，接下来我们来介绍一下代码块，代码块按照有无`static`可以分为**静态代码块**和**实例代码块**。
...
public class Singleton {
    ...
    // 3.定义一个静态方法返回这个唯一对象。要用的时候才实例化出对象
    public static Singleton getInstance() {
        if(instance == null) {
