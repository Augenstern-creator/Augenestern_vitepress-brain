...
![](09_Java_Stream流与字节流.assets/03-Overview.png)
...
![](09_Java_Stream流与字节流.assets/02-Lambda(2).png)
...
![](09_Java_Stream流与字节流.assets/02-流水线.jpeg)

当需要对多个元素进行操作（特别是多步操作）的时候，考虑到性能及便利性，我们应该首先拼好一个“模型”步骤方案，然后再按照方案去执行它。

![](09_Java_Stream流与字节流.assets/01-流式思想示意图(2).png)
...
- **相对路径**：相对于项目目录的路径，这是一个便捷的路径，开发中经常使用（重点）。
...
- `public boolean mkdirs()` ：创建由此File表示的目录，包括任何必需但不存在的父目录。（建议使用）
...
- `public String[] list()` ：返回一个String数组，表示该File目录中的所有子文件或目录。（只能拿到一级文件名称）

- `public File[] listFiles()` ：返回一个File数组，表示该File目录中的所有的子文件或目录。（重点）  
...
![](09_Java_Stream流与字节流.assets/1.png)
